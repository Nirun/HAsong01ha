...
<br />
    <font class="bottomtext">รายชื่อคนที่สมัครใน course นี้ (กรณี เป็นบุคคลที่ผู้อื่นสมัครให้ รายชื่อผู้นั้นจะอยู่ภายใต้ ชื่อคนที่สมัครให้) </font>
        ...
        <tr valign="middle" bgcolor="#a0cdf9">
            ...
            <td align="center" height="30"><p class="fronttext">รพ.</p> </td>
            <td align="center" height="30"><p class="fronttext">ประเภท<br>การสมัคร </p> </td>
            <td align="center" height="30"><p class="fronttext">จำนวน<br>สมัคร </p> </td>
            <td align="center" height="30"><p class="fronttext">วันที่สมัคร</p> </td>
            <td align="center" height="30"><p class="fronttext">สถานะ</p> </td>
            ...
            <td align="center" height="30"><p class="fronttext">จัดการ</p> </td>
        </tr>
        <?php
        $j = 1;
        foreach ($data[$i]["reg"] as $reg):
            if ($val['courseID'] == $reg['courseID']) {
                ...
                if($reg['IsPaid'] == 1 ){
                    $status = "ชำระเงินแล้ว";
                }else if ($reg['IsPaid'] == 2 || $reg['IsPaid'] == 0){
                    $status = "รอการชำระ";
                    $bgWait = "background-color:#FA9090";
                }else if ($reg['IsPaid'] == 4 ){
                    $status = "รอที่นั่งว่าง";
                }
                ...
                $days = round((strtotime($today) - strtotime($reg['ReQueue_Date'])) / (60 * 60 * 24));
                if($reg['ReQueue_Date'] == "0000-00-00 00:00:00" ){
                    if($reg['dateOver'] > 15 ){
                        $bgcolor = "background-color:#e1f0ff";
                        $isOver = 1;
                    }else{
                        $bgcolor = "background-color:white";
                        $isOver = 0;
                ...
                ?>
                <!--course details-->
                <tr valign="middle" style="<?php echo $bgcolor.';'; echo $bgWait; ?>">
                    <td align="center" class="info"><?php echo $j; ?></td>
                    ...
                    <td align="center" class="info"><?php echo $reg['member']; ?></td>
                    <td align="center" class="info"><?php echo $registeredDate; ?></td>
                    <td align="center" class="info"><?php echo $status; ?></td>
                    <td align="center" class="info"><?php echo $paiddate; ?></td>
                    <?php if($reg['IsPaid'] == 1 ){ ?>
                    ...
                    <?php }else if ($reg['IsPaid'] == 2 || $reg['IsPaid'] == 0 || $reg['IsPaid'] == 4){?>
                    ...
                    <td align="center" class="info">
                         <?php if($isOver == 1 ){ ?>
                            <?php //echo "Delete"; ?>
                            <input type="button"
                               onClick="if(confirm('<?php echo "ยืนยันการลบ กดปุ่มด้านล่างเพื่อยืนยันการลบรายชื่อนี้"; ?>')){location='<?php echo setting::$BASE_URL;?>/register/payment/delPayment/<?php echo $reg['registrationID'];?>/<?php echo $regTypeID;?>/<?php echo $traineeID;?>/<?php echo $reg['refID'];?>/del';}"
                               value="ลบ"/>
                         <?php }else { ?>
                            <?php //echo "Requeue"; ?>
                            <input type="button"
                               onClick="if(confirm('<?php echo "ระบบจะทำการส่งใบแจ้งเตือนการชำระเงินไปยังกลุ่มคนนี้"; ?>')){location='<?php echo setting::$BASE_URL;?>/register/payment/requeuePayment/<?php echo $reg['registrationID'];?>/<?php echo $regTypeID;?>/<?php echo $traineeID;?>/<?php echo $reg['refID'];?>/requeue';}"
                               value="ส่งใบชำระเงิน"/>
                         <?php } ?>
                    </td>
                    ...
                    <?php }else if ($reg['IsPaid'] == 1){?>
                    <td align="center" class="info">
                        <?php echo "ได้ที่นั่งแล้ว"; ?>
                    </td>
                    <?php }?>
                </tr>
                <!--course details-->
